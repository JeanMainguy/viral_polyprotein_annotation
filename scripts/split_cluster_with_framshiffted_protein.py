...
def split_cluster(cds_list):
    ...
    for i, group in cds_group.items():
        ...
        if len(group) == 1:
            logging.warning(
                'The splitting would make a singleton cluster.. the sequence is merged in the group of long sequence if possible')
...
def main():
    ...
    with open(cluster_file, 'r') as fl:
        for i, l in enumerate(fl):
            ...
            cds_list = analysis.getCdsObject(
                taxon_protein_ids, taxonomy_file, gff_file, sp_treshold)
            # print("=="*20)
            # print("CLUSTER ", i)
            new_lines = None
            ...
            if new_lines:
                ...
                assert sum((len(n_l.split('\t')) for n_l in new_lines.values())) == len(
                    l.split('\t')), 'sum elments in new cluster is different of nb element in old cluster!'
